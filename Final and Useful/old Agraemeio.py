...
def game(screen, background, colour,speed):
    ...
    xGPoints, yGPoints = coordinates(0,1700,0,900)
    xBPoints, yBPoints = coordinates(0,1700,0,900)
    ...
    while keepGoing:
        ...
        if speedChange > 5:
            ...
            xGPoints, yGPoints = coordinates(xStart,xStop,yStart,yStop)
            xBPoints, yBPoints = coordinates(xStart,xStop,yStart,yStop)
...
def coordinates(xStart,xStop,yStart,yStop):
    xPoints = []
    yPoints = []
    loop = 0
    while loop <5:
        for i in range(xStart,xStop,200):
            temp = random.randint(20+i,200+20+i)
            if temp < 910 or temp > 1010:
                xPoints.append(temp)


        for i in range(yStart,yStop,100):

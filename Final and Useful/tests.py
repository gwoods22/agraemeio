...
def game(screen, background, colour,speed):
    ...
    xGPoints, yGPoints = coordinates(200,100)
    xBPoints, yBPoints = coordinates(200,100)
    ...
    while countdown:
        ...
        if loop > 15:
            ...
            loop = 0
            if num > 1:
                music.load('files/beep.ogg')
                music.play()
            elif num == 1:
                music.load('files/finalBeep.ogg')
                music.play()
...
def coordinates(xStep,yStep):
    xPoints = []
    yPoints = []
    loop = 0
    while loop <5:
        for i in range(0,1700,xStep):
            temp = random.randint(20+i,xStep+20+i)
            if temp < 910 or temp > 1010:
                xPoints.append(temp)


        for i in range(0,900,yStep):
            temp = random.randint(20+i,yStep+20+i)
...
def instructions(screen):
    ...
    textBackground = pygame.Surface((400,300))
    ...
    while keep_going == True:
        ...
        if mouseY > 400 and mouseY < 500 and mouseX > 555 and mouseX < 666:
            ...
            pygame.draw.rect(blue,(0,0,255),((0,0), (110,100)))
        ...
        screen.blit(textBackground,(38,65))
        screen.blit(high,(625,125))


        for i in range(len(instruction_label)):
            screen.blit(instruction_label[i], (50, 30*i + 75))
